...
char ORI_READ_INFO_tag_names[20][3] = { "XA",  "MC", "SA", "NM" };
int ORI_READ_INFO_tag_number = 3;

char ORI_READ_INFO_tag_names_int[20][3] = { "NM" };
int ORI_READ_INFO_tag_number_int = 1;

void READ_SIGNAL_HANDLER::all_signal_records_read_pair(bam1_t &read1, bam1_t &read2, bool will_Be_used){

	bs.collect_signal(read1);
	bs.collect_signal(read2);
	...
	//full match:
	if(discard_both_full_match){ //full match
		int min_score = (b[0]->core.l_qseq + b[1]->core.l_qseq) * match - (4) * (match + mismatch);
		bool near_full_match = (score[0] + score[1] >= min_score);
		bool isize_right = (isize != 0 && isize > isize_min && isize < isize_max);
		if(discard_both_full_match && (near_full_match) && isize_right && ((tid[0] == tid[1] && tid[0] <= maxTid && tid[1] <= maxTid))) return;
	}
	...
	char filter_fail_reason[2][102400];
	int reason_len[2] = {0};
	...
	if((! pass_filter) || NOT_USING_FILTER){
		...
		bs.reasonFlagCounter[reasonFlag] ++;

		xassert(b[0]->core.l_qseq < MAX_ACCECPT_READ_LEN, "");

		if(!already_output_isize){
			sprintf(filter_fail_reason[0] + reason_len[0], "STAT_%d_%d_%d_%d_", bs.analysis_read_length, bs.minInsertLen, bs.middleInsertLen, bs.maxInsertLen);
			reason_len[0] += strlen(filter_fail_reason[0] + reason_len[0]);
			already_output_isize = true;
		}

		//output all other read informations in the command tags
		//flags + mapq + CIGAR + all tags
		//need to be debug
		for(int read_id = 0; read_id < 2; read_id++){
			//for basic part
			sprintf(filter_fail_reason[read_id] + reason_len[read_id], "FLAG_%d_%d_CIGAR_", b[read_id]->core.flag, b[read_id]->core.qual);
			reason_len[read_id] += strlen(filter_fail_reason[read_id] + reason_len[read_id]);
			//for cigar:
			uint32_t* bam_cigar = bam_get_cigar(b[read_id]);
			for (uint cigar_id = 0; cigar_id < b[read_id]->core.n_cigar; ++cigar_id)
			{
				int type = (int)((bam_cigar[cigar_id] & BAM_CIGAR_MASK));
				int length = (bam_cigar[cigar_id] >> BAM_CIGAR_SHIFT);
				sprintf(filter_fail_reason[read_id] + reason_len[read_id], "%d%c",  length, "MIDNSHP=XB"[type]);
				reason_len[read_id] += strlen(filter_fail_reason[read_id] + reason_len[read_id]);
			}
			sprintf(filter_fail_reason[read_id] + reason_len[read_id], "_");
			reason_len[read_id] ++;

			//for mate information
			sprintf(filter_fail_reason[read_id] + reason_len[read_id], "MATE_%d_%d_%d_", b[read_id]->core.mtid, b[read_id]->core.mpos, b[read_id]->core.isize);
			reason_len[read_id] += strlen(filter_fail_reason[read_id] + reason_len[read_id]);

			//for tags
			sprintf(filter_fail_reason[read_id] + reason_len[read_id], "TAG_");
			reason_len[read_id] += strlen(filter_fail_reason[read_id] + reason_len[read_id]);
			//SA & XS & XA
			for(int tag_id = 0; tag_id < ORI_READ_INFO_tag_number; tag_id++){
				const char *tag_Str = bam_get_string_tag(b[read_id], ORI_READ_INFO_tag_names[tag_id]);
				if(tag_Str != NULL){
					sprintf(filter_fail_reason[read_id] + reason_len[read_id], "%s:Z:%s_", ORI_READ_INFO_tag_names[tag_id], tag_Str);
					reason_len[read_id] += strlen(filter_fail_reason[read_id] + reason_len[read_id]);
				}
			}
			//NM
			for(int tag_id = 0; tag_id < ORI_READ_INFO_tag_number_int; tag_id++){
				int32_t tag_num = 0;
				if(bam_get_num_tag(b[read_id], ORI_READ_INFO_tag_names_int[tag_id], &tag_num)){
					sprintf(filter_fail_reason[read_id] + reason_len[read_id], "%s:i:%d_", ORI_READ_INFO_tag_names_int[tag_id], tag_num);
					reason_len[read_id] += strlen(filter_fail_reason[read_id] + reason_len[read_id]);
				}
			}
		}
...
void mate_fail_handler(int &total_unpaired_read_number, bam1_t *cr, int i, int true_load_size, const char *func, const int LINE){
	...
	if(total_unpaired_read_number % 1000 == 0){
...
{
	bs.total_read_number = 0;
	//unpaired tmp files
	char tmp_fn [1024];	sprintf(tmp_fn, "%s.tmp.bam", tmp_file_pairing);
	...
	for(int i = 0; i < true_load_size; i+=2){
		...
		bool namesAreSame = (0 == strcmp((char *)bam_qname(b1), (char *)bam_qname(b2)));
		if(namesAreSame == false){
			fprintf(stderr, "Read Pairing error!, reads names are not same, please confirm the completeness of BAM/CRAM file.\n");
			i--; continue;
		}
...
void READ_SIGNAL_HANDLER::SURVIVOR_SV_region_get_all_signal_records_SORT_BY_NAME()
{
	...
	while (1){
		...
		bs.total_read_number += 2;
		if(bs.total_read_number % 100000 == 0)
			fprintf(stderr, "%ld\r", bs.total_read_number);
